...
export default function SetupPage() {
  ...
  const [industry, setIndustry] = useState<Industry>('self-storage');
  const [role, setRole] = useState<PrimaryRole>('developer');
  const [config, setConfig] = useState<OrgConfig>(defaultConfigForIndustry('self-storage', 'developer'));
    ...
    async function load() {
      try {
        ...
        if (org) {
          ...
          // Pre-fill from existing partial config if any
          if (org.settings?.industry) {
            setIndustry(org.settings.industry);
            setConfig({ ...org.settings, onboarding_completed: false });
          }
        }
      } catch {
